...
export const GET = async () => {
  try {
    ...
    // Fetch unread messages and sort by createdAt in descending order (newest first)
    const unreadMessages = await Message.find({ recipient: userId, read: false })
      .sort({ createdAt: -1 }) // Sort unread messages in descending order
      .populate('sender', 'username')
      .populate('property', 'name');

    // Fetch read messages and sort by createdAt in descending order (newest first)
    const readMessages = await Message.find({ recipient: userId, read: true })
      .sort({ createdAt: -1 }) // Sort read messages in descending order
      .populate('sender', 'username')
      .populate('property', 'name');

    // Combine unread messages (at the top) and read messages (at the bottom)
